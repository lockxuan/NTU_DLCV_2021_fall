...
from p1_utils import cal_distance
...
def predict(args, model, data_loader):
    distance = cal_distance(args.distance, model)
    prediction_results = []
    with torch.no_grad():
        ...
        for i, (data, target) in enumerate(data_loader):
            ...
            prototype = prototype.reshape(args.N_way, args.N_shot, -1) # (n_way, k_shot, out_dim)
            ...
            prediction_results.append(preds.detach().cpu().numpy())
            print('step: {}/{}'.format(i+1, len(data_loader)), end='\r')

    prediction_results = np.stack(prediction_results, axis=0)
...
def parse_args():
    ...
    parser.add_argument('--distance', default='euclidean', type=str, help="distance method")
...
if __name__=='__main__':
    ...
    with open(args.output_csv, 'w', newline='') as csvfile:
        ...
        row_title = ['episode_id']
        for i in range(75):
            row_title.append('query' + str(i))
        writer.writerow(row_title)

        for i, preds in enumerate(prediction_results):
            preds = np.insert(preds, 0, i)
